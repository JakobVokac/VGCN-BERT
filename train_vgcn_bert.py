...
with open("data/tweets/test.txt", "r+") as f:
    ...
    for i, raw in enumerate(f.readlines()):
        raw.lower()
        dataset.append(InputExample(i, raw))

    predictions, confidences = predict(model, examples, tokenizer, 1000)

    output = open("data/submission.csv", "w+")
    output.write("Id, Prediction\n")
    for i in range(0, len(predictions)):
        output.write(str(i+1)+", "+predictions[i]+"\n")
    output.close()